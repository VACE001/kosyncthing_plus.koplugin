...
-- ─────────────────────────────────────────────────────────────────────────────
-- The Kindle scanner passes IgnoreRegistry patterns to `find` as `! -name`.
-- The Android lfs scanner must apply the same exclusions, or a companion's own
-- conflict files would be miscounted in the badge.
describe("IgnoreRegistry exclusions (companion parity)", function()
    it("translates -name globs to anchored basename patterns", function()
        local g = A._globToLuaPattern
        assert.is_not_nil(("a.x"):match(g("*.x")))
        assert.is_not_nil(("book.sync-conflict-syncery.epub"):match(g("*syncery*")))
        assert.is_nil(("a.sdr"):match(g("*.epub")))
        assert.is_nil(("aXb"):match(g("a.b")))   -- the dot is literal, not "any char"
    end)

    it("filters out conflict files matching a registered exclusion", function()
        local root = mktemp()
        os.execute("touch '" .. root .. "/book.sync-conflict-20260101-A.epub'")
        os.execute("touch '" .. root .. "/note.sync-conflict-20260202-syncery.sdr'")
        local pl = {
            showNotification = function() end,
            getFolders = function() return { { path = root } } end,
            _android_exclusions_fn = function() return { syncery = "*syncery*" } end,
        }
    ...
    it("counts all conflicts when there are no exclusions", function()
        local root = mktemp()
        os.execute("touch '" .. root .. "/a.sync-conflict-20260101-A.epub'")
        os.execute("touch '" .. root .. "/b.sync-conflict-20260202-syncery.sdr'")
        local pl = { showNotification = function() end, getFolders = function() return { { path = root } } end }
