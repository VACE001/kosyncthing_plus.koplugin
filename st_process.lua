...
local function binaryExists(self)
    ...
    if not f then
        -- Could not open for reading (permission denied, filesystem error,
        -- or the path is a directory).  Treat as "no binary" and cache the
        -- result so we don't retry every call.
        logger.warn("[Syncthing] binaryExists: cannot open " .. p)
        _binary_exists_cache = false
        return false
    end
    ...
    if not magic or magic ~= "\x7fELF" then
