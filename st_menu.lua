    ...
    return {
        ...
        -- also copies it to the clipboard as a backup.
        {
			...
			callback = self.safe("Diagnostic snapshot", function()
				...
				-- Binary file — check if the binary is a valid ELF executable.
				-- A text file named "syncthing" (Kobo appstream metadata) was
				-- historically mistaken for the real binary.
				ln("=== Binary file ===")
				...
				if not util.pathExists(bin_path) then
					...
					if lfs then
						...
						if attr and attr.size then
							ln("Size:       " .. tostring(attr.size) .. " bytes")
						end
					end
				end
				ln("")

				-- Process — only shown when Syncthing is running (PID known).
				if pid then
					ln("=== Process ===")
					local f = io.open("/proc/" .. pid .. "/status", "r")
					if f then
						for line in f:lines() do
							if line:find("^VmRSS:") then
								ln("RSS:        " .. line:gsub("^VmRSS:%s*", ""))
							elseif line:find("^Threads:") then
								ln("Threads:    " .. line:gsub("^Threads:%s*", ""))
							end
						end
						f:close()
					end
					...
					if sf then
						...
						if stat then
							...
							if after_paren then
								...
								local total_sec = (utime + stime) / 100
								ln("CPU time:   " .. string.format("%.1f s", total_sec))
							end
						end
					end
					ln("")
				...
				if free_plugin then
					ln("Free space: " .. U.formatBytes(free_plugin))
				end
				ln("")

				-- Network — loopback state, device IP, and Kindle port status.
				ln("=== Network ===")
				ln("Loopback:    " .. (U.loopbackIsUp() and "up" or "down"))
				local ip = U.getDeviceIP()
				if ip and ip ~= "127.0.0.1" then
					ln("Device IP:   " .. ip)
				end
				-- On Kindle, check whether the sync and discovery ports are open in iptables.
				-- These are needed for pairing and data transfer; they are often blocked by default.
				if Device:isKindle() then
					...
					ln("Sync port:   " .. (sync_open and "open (TCP 22000)" or "blocked (TCP 22000)"))
					ln("Discovery:   " .. (disc_open and "open (UDP 21027)" or "blocked (UDP 21027)"))
				end
				ln("")

				-- Configuration — folder/device counts and GUI auth status.
				ln("=== Configuration ===")
				...
				if not util.pathExists(config_xml) then
					...
					if f then
						...
						if content and not (content:find("<user>") and content:find("<password>")) then
							ln("GUI auth:    NOT configured (no password set)")
						end
					end
				end
				ln("")
